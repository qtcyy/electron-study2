...
class FoldWatcher {
  ...
  initFiles() {
    try {
      const files = fs$1.readdirSync(this.watchPath);
      files.forEach((file) => {
  ...
  startWatch() {
    ...
    this.watcher = fs$1.watch(this.watchPath, (eventType, filename) => {
      if (filename && eventType === "rename") {
        const filePath = path$1.join(this.watchPath, filename);
        fs$1.access(filePath, fs$1.constants.F_OK, (err) => {
...
async function fileOpen(rootPath, filename) {
  const filePath = path$1.join(rootPath, filename);
...
class FileUpload {
  ...
  async upload(filename) {
    const filePath = path$1.join(this.rootPath, filename);
    console.log(filePath);
    const file = await fs$2.readFile(filePath);
    await axios.post(this.api, file);
  }
}
createRequire(import.meta.url);
const __dirname = path$2.dirname(fileURLToPath(import.meta.url));
const watchPath = "/Users/chengyiyang/Desktop/文档文件/test-watch/";
